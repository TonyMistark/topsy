...
class AddUserToBoardTestCase(TestCase):
    ...
    def test_add_user_to_board(self):
        ...
        request = self.create_request(
            {"user_id": user.id, "board_id": board.id, "role": "editor"}, user=owner
        )
    ...
    def test_no_permission_to_add_user_to_board(self):
        ...
        request = self.create_request(
            {"user_id": user.id, "board_id": board.id, "role": "editor"}, req_user
        )
        response = add_user_to_board(request)
...
class RemoveUserFromBoardTestCase(TestCase):
    ...
    def test_remove_user_from_board_success(self):
        ...
        request = self.create_request(
            {"board_id": board.id, "user_id": reader.id}, user=owner
        )
...
class GetNoteTestCase(TestCase):
    ...
    def test_get_note_permission_denied(self):
        ...
        self.assertEqual(
            response.status_code, 403, "Error: {}".format(response.content)
        )
